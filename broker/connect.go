...
func (c *ConnectPacket) UpdateSession() error {
	...
	client.KeepAlive = c.KeepAlive
	client.CleanSession = c.CleanSession
	...
	if c.Will != nil {
		mes.QOS = c.Will.QOS
		mes.Retain = c.Will.Retain
		mes.Topic = c.Will.Topic
		mes.Payload = c.Will.Payload
		db.Save(&mes)
	}
	db.Save(&client)
	return nil
...
func (cp *ConnectPacket) Decode(src []byte) (int, error) {
	...
	if cp.Will != nil {
		...
		cp.Will.Payload, n, err = readLPString(src[total:], cp.Type())
		total += n
		if err != nil {
			return total, err
		}
...
func (cp *ConnectPacket) Encode(dst []byte) (int, error) {
	...
	if cp.Will != nil {
		...
		n, err = writeLPString(dst[total:], cp.Will.Payload, cp.Type())
		total += n
		if err != nil {
			return total, err
		}
